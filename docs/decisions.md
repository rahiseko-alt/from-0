...
| サブエージェントは空にしない。`failure-reviewer` を置く    | **判断** | 上記の「空にする」判断を一部撤回した。理由は「9. 失敗台帳・放置台帳」を参照。中身のないサンプルではなく、失敗台帳を読むために必要な機能のため                          |
...
| 項目                                                                                | 理由 / 変更してよい条件                                                                                                                                                                                          |
| ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docs/failure-log.md` を作る。書きっぱなしでよい                                    | 失敗のたびに整理を判断させるとその判断自体の仕組みが要る。書く側の負担をゼロにし、整理コストは読む側（`failure-reviewer`）に寄せた                                                                               |
| 通常のセッションでは失敗台帳を読まない                                              | コンテキストを圧迫するため。読むのはユーザーの明示的指示、または AI が繰り返し失敗に気づいてユーザーに確認しYesを得たときだけ                                                                                    |
| 失敗台帳を読む前に必ずユーザーへ確認する                                            | AI がサブエージェントを自分の判断だけで呼び出せると、呼び出し基準が曖昧なまま暴走しうる。ユーザーの承認を必須にした                                                                                              |
...
| `failure-reviewer` サブエージェントを追加                                           | 「1. 指示ファイルの構成」で一度見送った「サブエージェントを空にする」方針を、この機能に限り撤回した。中身のないサンプルではなく、失敗台帳の整理という具体的な必要から追加した                                    |

### 保留にした部分

Task Graph（計画の構造）／Runtime State（今どのタスクが進行中・完了・失敗かの機械的な記録）／
セッション開始時に計画と現在地を自動把握する仕組みは、今回はまだ実装していません。
具体的なファイル形式・スキーマがまだユーザーと合意できていないため。次回以降の検討課題です。
