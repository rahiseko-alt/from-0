...
- `verify` の1つ目（`AGENTS.md` に手順がある）は済んでいる
- 2つ目（実際に「満たした」と判定されるのを見る）が**構造的に観測できない**。`/goal` は
  判定される側から起動できず、しかも「全項目完了」を条件にすると `T006` 自身が最後の1件に
  なるため、循環する
- 進め方は次のセッションの判断に委ねる。**別の小さな項目で `/goal` を1回試すのが素直**
