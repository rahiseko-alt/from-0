...
- **ゴール到達時の振り返りをまだやっていない。** `docs/neglected-log.md` の4件を読み、
  記録された日時・Gate・対象箇所を手がかりに実際に確認しに行く（`AGENTS.md`「失敗と放置の記録」）
- **`/plan-verify`（全体照合）を一度も回していない。** 10項目ごとに回す決まりだが、
  このセッションでは27項目を一気に進めたため未実施。**「済」にした項目が後から壊れていないか、
  次のセッションで確認するのが素直**
