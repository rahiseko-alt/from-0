...
| 作業終了（チェックアウト） | `/checkout`  | 引継ぎを更新し、検証を通し、**`main` へマージするまで**完了させる |

> **引継ぎは `main` に載せて初めて引き継がれます。**
> セッションのコンテナは終了後に破棄され、次のセッションは `main` を新規クローンします。
> 作業ブランチにコミットしただけでは失われます。「PR を出した」で止めないでください。
