...
- **週に1度、Routine が進み具合と見送り件数を報告する。** 全項目が終わると「ゴールに到達
  しました」と明記される（それが振り返りの合図）。**いまは通知が手元に届かないため、
  claude.ai の Routines 画面で報告を開いて読む** `[曖昧]`（原因はリポジトリの外）。
  止めたい・間隔を変えたいときも同じ画面で操作する
