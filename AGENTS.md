...
- **失敗したら `docs/failure-log.md` に書く。書きっぱなしでよい。** 自由記述で構わないが、
  日時は必須。通常のセッションでは読まない（コンテキストを圧迫するため）。読むのはユーザーが
  明示的に指示したとき、または同じ失敗を繰り返していると気づいたときだけで、後者は読む前に
  必ずユーザーへ「失敗台帳を読みますか？」と確認する。読むときは自分で読まず
  `failure-reviewer` サブエージェントに委譲する
