...
| タイミング                 | 実行するもの | 内容                                                                                            |
| -------------------------- | ------------ | ----------------------------------------------------------------------------------------------- |
| 作業開始（チェックイン）   | `/checkin`   | `main` を同期し、引継ぎと未処理の PR を確認して次の一手を出す                                   |
| 作業の区切り               | `/handoff`   | 引継ぎを更新してコミットする                                                                    |
| 作業終了（チェックアウト） | `/checkout`  | **引継ぎ文**を更新し、検証を通し、**`main` へマージするまで**完了させる（コードは下記のとおり） |

> **`main` へ必ずマージするのは「引継ぎ文」です。実際のコード変更は、未完了なら
> push 済みの branch/worktree に残したままで構いません。**
> 次のセッションは SessionStart で `main` の `docs/handoff.md` しか自動では読まないため、
> 「未完了・branch X で継続中」という引継ぎ文自体が `main` に無いと、次のセッションは
> 未完了の作業があること自体に気づけません。一方、push 済みのコミットは origin に残るため
> 失われません（失われるのは **push していない** コミットだけです）。
> 「PR を出した」で止めず、**引継ぎ文の PR** はマージまで進めてください。

未コミットの変更が残っていると Stop フックが**1セッションに1回だけ**更新を求めます。
セッション終了時にはフックがブランチと HEAD と変更ファイルを末尾に自動記録します。
...
.claude/skills/        スキル。/checkin /checkout /handoff の3つ
